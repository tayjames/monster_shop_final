...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @order_1 = @user.orders.create!(address: @home, status: "packaged")
      @order_2 = @user.orders.create!(address: @home, status: "pending")
  ...
  describe 'class methods' do
    before :each do
      @user = User.create!(name: 'Megan', email: 'megan_1@example.com', password: 'securepassword')
      @home = @user.addresses.create!(street: "1111 Ash St.", city: "Denver", state: "CO", zip: "80220")
      @order_1 = @user.orders.create!(address: @home, status: 1)
      @order_2 = @user.orders.create!(address: @home, status: 0)
      @order_3 = @user.orders.create!(address: @home, status: 3)
      @order_4 = @user.orders.create!(address: @home, status: 2)
    end
